    ...
    while remaining_processes or ready_queue:
        ...
        if ready_queue:
            ...
            if process['remaining_time'] > 0:
                current_time += overhead  # Adiciona sobrecarga somente se o processo ainda precisa ser executado
                ready_queue.append(process)
...
def edf_scheduler(processes, quantum, overhead):
    ...
    while sorted_processes or ready_queue:
        while sorted_processes and sorted_processes[0]['arrival_time'] <= current_time:
            ready_queue.append(sorted_processes.pop(0))
            ready_queue.sort(key=lambda x: x['deadline'])  # Mantém ordenado pelo deadline a cada novo processo
        ...
        if ready_queue:
            ...
            if process['execution_time'] > 0:
                current_time += overhead  # Adiciona sobrecarga apenas se o processo voltar à fila
                ready_queue.append(process)
                ready_queue.sort(key=lambda x: x['deadline'])  # Reordena para priorizar menor deadline
            else:
                completion_time = current_time  # Não adiciona overhead ao terminar
